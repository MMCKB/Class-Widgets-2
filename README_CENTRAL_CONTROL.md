...
本指南说明如何用 **GitHub Pages + GitHub Actions** 为 Class Widgets 2（以下简称 **CW**）部署拉取式集控。当前集控可以安全地下发多份经 SHA-256 校验的课程表，并向客户端发送一次性公告通知。客户端默认只同步和保存课表；只有**恰好一份**下发课表在自身 JSON 根对象中声明 `yes.id: 1` 时才会自动切换。全部为 `0`、缺失该参数，或同时有两份及以上为 `1` 时，客户端一律保持当前课表；它不是常驻服务，也不支持服务器主动推送、远程重启或执行任意命令。[1] [3]
...
先在 CW 中导出或准备一份或多份**当前客户端能够正常读取的课程表 JSON**，保存到仓库根目录独立的 `课程表/` 文件夹。该文件夹是课程表唯一允许的存放位置，不要再使用旧的英文课程表目录。不要手工猜测课程表结构；每份课程表都会由客户端按当前 `ScheduleData` 模型和 `meta.version` 校验。[3]

### 2. 计算课程表 SHA-256

`manifest.json` 必须为每份课程表写入当前字节内容的 SHA-256。任一课程表改动，包括仅重新格式化 JSON，都要重新计算该文件哈希并更新清单。[3]

| 系统 | 命令 |
|---|---|
| Linux / macOS / Git Bash | `sha256sum 课程表/class-a.json` |
| Windows PowerShell | `Get-FileHash .\课程表\class-a.json -Algorithm SHA256` |
...
```bash
git add manifest.json "课程表/class-a.json" "课程表/class-b.json"
...
| `schedules` | 是 | 课程表描述数组，至少 1 份、最多 20 份。旧版单个 `schedule` 字段仍可兼容读取。 |
| `schedules[].id` | 是 | 同一清单中必须唯一；仅允许字母、数字、`-`、`_`，长度 1–64；本地缓存名为 `central_<id>.json`。 |
| `schedules[].name` | 建议 | 设置页中显示的课程表名称。 |
| `schedules[].url` | 是 | 课程表 JSON 的相对或绝对 HTTP(S) 地址。 |
| `schedules[].sha256` | 是 | 对应课程表内容的 64 位小写十六进制 SHA-256。 |
| `schedules[].scheduleSchemaVersion` | 建议 | 便于管理员记录课程表版本；实际以课程表 JSON 内的 `meta.version` 为准。 |
| 课程表 JSON 根对象 `yes.id` | 可选 | 仅允许整数 `0` 或 `1`；`1` 表示该课表请求自动切换，`0` 或缺失表示仅下发保存。客户端只接受恰好一份为 `1` 的情况。 |
| `commands` | 否 | 公告命令数组；省略或设为 `[]` 表示不下发公告。 |

客户端会逐份验证清单版本、课程表标识、HTTP 状态、课程表大小（每份最多 2 MB）、SHA-256、UTF-8 JSON、课程表架构版本，以及可选的 `yes.id` 是否为 `0` 或 `1`。任一份课表或公告命令验证失败时，客户端会静默拒绝整次下发：不写入课表、不切换、不执行同批公告、不产生用户可见通知或失败提示，并保留上次成功同步的内容。[3]

### `yes.id` 自动切换参数

`yes` 是下发课程表 JSON 根对象中的可选扩展参数，不写入 `manifest.json`。管理员可在导出的课表 JSON 顶层添加：

```json
{
  "yes": {"id": 1},
  "meta": {"id": "...", "version": 1, "maxWeekCycle": 20, "startDate": "2026-09-01"}
}
```
...
|---:|---|
| `1` | 将该课表列入自动切换候选。 |
| `0` 或缺失 | 仅下载、校验并保存为可选课表。 |
| 其他值或非整数 | 静默拒绝本次集控下发；不应用课表或公告，也不产生用户可见提示。 |

当且仅当整份清单中只有一份课表的 `yes.id` 为 `1` 时，客户端自动切换。若所有课表都不是 `1`，或两份及以上同时为 `1`，客户端拒绝自动切换并在设置页显示状态；用户仍可手动点击“应用”切换任意已同步课表。

### 使用工作流加入与选择课表

集控仓库可包含 `.github/scripts/add_schedule_to_manifest.py` 和 `.github/workflows/add-schedule-to-manifest.yml`。管理员把课程表放入 `课程表/` 后，先在仓库网页打开 **Actions → 加入集控课程表 → Run workflow**，自行填写顶层 JSON 文件名与可选显示名称。该工作流只会将指定文件的 URL、SHA-256、架构版本和显示名称写入 `manifest.json`；不会改动课程表 JSON 或其 `yes.id`。未加入清单的课程表可留在仓库中备用，但 CW 客户端不会下载它。

用于自动切换的集控仓库应包含 `.github/scripts/select_schedule.py` 和 `.github/workflows/select-schedule.yml`。管理员在仓库网页打开 **Actions → 下发切换课程表 → Run workflow**，并在输入框中**自行填写** `课程表/` 顶层的一个 JSON 文件名，例如 `class-a.json`。

工作流会拒绝路径、`../`、绝对路径和非 JSON 输入；它只处理 `课程表/` 内文件。运行后，所选文件写入 `"yes": {"id": 1}`，同一文件夹中其余课程表写入 `"yes": {"id": 0}`，随后重建 `manifest.json` 的 `schedules`、逐份计算 SHA-256 并创建可审计提交。它不会根据名称或设备信息自动猜测选择哪份课表。

> 首次迁移到 `课程表/` 时，建议先使用脚本的 `--clear-selection` 参数将所有课表设为 `yes.id=0`，再由管理员运行工作流选择一份。这样能避免迁移期间意外自动切换。
...
cp /path/to/exported-schedule.json "课程表/class-a.json"

# 2) 重新计算该文件的 SHA-256
sha256sum "课程表/class-a.json"

# 3) 将输出的哈希写入 manifest.json 对应 schedules[] 项的 sha256
# 4) 增加 policyVersion，例如 2026-08-26-002
# 5) 提交和推送
git add "课程表/class-a.json" manifest.json
git commit -m "feat: update central-control schedule"
git push
```

拉取成功后，CW 使用原子写入把每份课程表保存到本地课程表目录，命名为 `central_<schedules[].id>`。用户可随时在集控页面点击某份课表的“应用”按钮。自动切换只看课程表自身的 `yes.id`：仅有一份为 `1` 时切换；全为 `0` 或缺失时只下发；两份及以上为 `1` 时拒绝切换。[3]
...
| 模式 | 操作 | 网络行为 |
|---|---|---|
| 手动拉取 | 关闭“自动拉取集控内容”，点击“检查并同步集控内容” | 仅点击时请求网络；同步后由用户明确点击“应用”切换。 |
| 自动拉取 | 开启“自动拉取集控内容”，设置 1–1440 分钟间隔 | 启动时和指定间隔检查并保存课表；仅有一份课表 `yes.id=1` 时切换。 |
| `yes.id` 自动切换 | 在下发课表 JSON 根对象写入 `"yes": {"id": 1}` | 该课表成为自动切换候选；无候选或多个候选时均拒绝切换。 |

建议先在**手动拉取**模式下验证地址、SHA-256 与 `yes.id`，再按设备规模设置自动间隔。发布前确认同一清单中不要有两份及以上课表同时写入 `yes.id: 1`；若地址为空、不是 `http://`/`https://`，或服务器返回无效内容，设置页会显示失败原因。[3]
...
## 通过 GitHub Actions 发布一次性命令

### 公告命令格式

客户端接受 `announcement` 公告命令和受限的 `power` 电源命令。每台客户端会把已处理命令的 ID 记录在本地配置中，因此**同一个 ID 只会处理一次**；最多保留最近 100 个已处理 ID。使用新的 ID 即可让一次新的命令在每台尚未处理该 ID 的设备上生效。[3]
...
电源命令只能使用 `restart`、`shutdown`、`hibernate`、`sleep` 四个固定动作，且每次下发最多一条。命令必须有 ISO 8601 过期时间；不接受 Shell、Python、程序路径、附加参数或任何自由文本命令。

```json
{
  "id": "power-20260826-001",
  "type": "power",
  "action": "restart",
  "expiresAt": "2026-08-26T14:10:00Z"
}
```

客户端默认拒绝该类型。只有在 **集控 → 远程电源命令** 中手动打开“允许接收远程电源命令”后，客户端才会接收并立即执行命令。未授权、过期或重复的命令不会执行。

集控仓库可加入 `.github/scripts/publish_power_command.py` 与 `.github/workflows/publish-power-command.yml`。管理员在 **Actions → 下发集控电源命令 → Run workflow** 中选择固定动作并输入“确认”后下发；工作流会自动写入唯一 ID 和 10 分钟有效期。
...
python -m json.tool "课程表/class-schedule.json" >/dev/null
sha256sum "课程表/class-schedule.json"
```
...
| HTTP 状态 | `200 OK`。 |
| `schemaVersion` | 为 `1`。 |
| `schedules[].url` | 每份课程表都能从浏览器直接访问或可由相对路径解析。 |
| `schedules[].sha256` | 每份课程表哈希都与其对应文件完全一致。 |
