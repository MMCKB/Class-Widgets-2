...
PLUGIN_AUTOMATION_PREFIX = "plugin."
...
class AutomationTrigger(BaseModel):
    ...
    def validate_type(cls, value: str) -> str:
        if value not in TRIGGER_TYPES and not value.startswith(PLUGIN_AUTOMATION_PREFIX):
...
class AutomationAction(BaseModel):
    ...
    @classmethod
    def validate_type(cls, value: str) -> str:
        if value not in ACTION_TYPES and not value.startswith(PLUGIN_AUTOMATION_PREFIX):
...
@dataclass
class PluginAutomationTrigger:
    """由本地插件注册、只能由该插件主动发出的触发器。"""
...
class AutomationProfilesService(AutomationTask):
    ...
    def __init__(self, app_central: "AppCentral") -> None:
        ...
        self._plugin_triggers: dict[str, PluginAutomationTrigger] = {}
        self._plugin_actions: dict[str, PluginAutomationAction] = {}
    ...
    @Property(list, notify=changed)
    def triggerOptions(self) -> list[dict[str, str]]:
        builtin = [
            {"id": "app_started", "title": self.tr("应用启动时")},
            {"id": "app_exiting", "title": self.tr("应用退出时")},
            {"id": "process_started", "title": self.tr("进程启动时")},
            {"id": "process_running", "title": self.tr("进程运行时")},
            {"id": "process_exited", "title": self.tr("进程退出时")},
            {"id": "class_started", "title": self.tr("上课时")},
            {"id": "break_started", "title": self.tr("课间时")},
            {"id": "school_dismissal", "title": self.tr("放学时")},
            {"id": "noon_dismissal", "title": self.tr("中午放学时")},
        ]
        return [*builtin, *(trigger.to_view() for trigger in self._plugin_triggers.values())]

    @Property(list, notify=changed)
    def actionOptions(self) -> list[dict[str, str]]:
        builtin = [
            {"id": "notification", "title": self.tr("显示提醒")},
            {"id": "run_program", "title": self.tr("运行程序")},
        ]
        return [*builtin, *(action.to_view() for action in self._plugin_actions.values())]
    ...
    def register_plugin_trigger(
        ...
        """注册只能由所属本地插件主动发出的自动化触发器。"""
        plugin_id = self._validate_plugin_project_component(plugin_id, "plugin ID")
        trigger_id = self._validate_plugin_project_component(trigger_id, "trigger ID")
        title = str(title).strip()[:80]
        if not title:
            raise ValueError("Plugin automation trigger title cannot be empty")
        qualified_id = f"{PLUGIN_AUTOMATION_PREFIX}{plugin_id}.{trigger_id}"
        if qualified_id in self._plugin_triggers:
            raise ValueError(f"Plugin automation trigger is already registered: {qualified_id}")
        self._plugin_triggers[qualified_id] = PluginAutomationTrigger(
            id=qualified_id,
            plugin_id=plugin_id,
            title=title,
            description=str(description).strip()[:240],
        ...
        self.changed.emit()
        logger.info("Registered plugin automation trigger: {}", qualified_id)
        return qualified_id

    def emit_plugin_trigger(self, plugin_id: str, trigger_id: str) -> bool:
        """由插件本地代码发出其已注册的触发事件。"""
        plugin_id = self._validate_plugin_project_component(plugin_id, "plugin ID")
        trigger_id = self._validate_plugin_project_component(trigger_id, "trigger ID")
        return self.emit_registered_plugin_trigger(f"{PLUGIN_AUTOMATION_PREFIX}{plugin_id}.{trigger_id}")

    def emit_registered_plugin_trigger(self, qualified_id: str) -> bool:
        """发出已注册的全局插件触发器 ID；未注册项不会进入规则引擎。"""
        qualified_id = str(qualified_id).strip()
        if qualified_id not in self._plugin_triggers:
            return False
        if self._running:
            self._dispatch_event(qualified_id)
        return True
    ...
    def register_plugin_action(
        ...
        """注册命中规则后调用的本地插件行动回调。"""
        plugin_id = self._validate_plugin_project_component(plugin_id, "plugin ID")
        action_id = self._validate_plugin_project_component(action_id, "action ID")
        title = str(title).strip()[:80]
        if not title:
            raise ValueError("Plugin automation action title cannot be empty")
        if not callable(on_execute):
            raise TypeError("Plugin automation action callback must be callable")
        qualified_id = f"{PLUGIN_AUTOMATION_PREFIX}{plugin_id}.{action_id}"
        if qualified_id in self._plugin_actions:
            raise ValueError(f"Plugin automation action is already registered: {qualified_id}")
        self._plugin_actions[qualified_id] = PluginAutomationAction(
            id=qualified_id,
            plugin_id=plugin_id,
            title=title,
            description=str(description).strip()[:240],
        ...
        self.changed.emit()
        logger.info("Registered plugin automation action: {}", qualified_id)
        return qualified_id

    def unregister_plugin_projects(self, plugin_id: str) -> None:
        """移除插件项目、触发器和行动的运行时注册；不保留可执行回调。"""
        project_ids = [project_id for project_id, project in self._plugin_projects.items() if project.plugin_id == plugin_id]
        trigger_ids = [trigger_id for trigger_id, trigger in self._plugin_triggers.items() if trigger.plugin_id == plugin_id]
        action_ids = [action_id for action_id, action in self._plugin_actions.items() if action.plugin_id == plugin_id]
        for project_id in project_ids:
            self._plugin_projects.pop(project_id, None)
        for trigger_id in trigger_ids:
            self._plugin_triggers.pop(trigger_id, None)
        for action_id in action_ids:
            self._plugin_actions.pop(action_id, None)
        if project_ids or trigger_ids or action_ids:
            self.changed.emit()
            logger.info(
                "Unregistered plugin automation extensions for {}: {} project(s), {} trigger(s), {} action(s)",
                plugin_id,
                len(project_ids),
                len(trigger_ids),
                len(action_ids),
    ...
    def _run_action(self, profile: AutomationProfile, rule: AutomationRule, action: AutomationAction) -> bool:
        if action.type == "notification":
            ...
            return True
        plugin_action = self._plugin_actions.get(action.type)
        if plugin_action is not None:
            try:
                result = plugin_action.on_execute(
                    {
                ...
                logger.warning("Plugin automation action failed for {}: {}", action.type, exc)
                return False
        if action.type.startswith(PLUGIN_AUTOMATION_PREFIX):
            logger.warning("Skipping unavailable plugin automation action: {}", action.type)
            return False
