...
class AppCentral(QObject):  # Class Widgets 的中枢
    ...
    def __init__(self, pending_plugin: str | None = None) -> None:  # 初始化
        super().__init__()
        ...
        # 从文件关联启动时传入的待安装插件路径
        self._pending_plugin: str | None = pending_plugin
    ...
    def init(self) -> None:
        ...
        self._schedule_pending_plugin_prompt()

    @Slot()
    def _schedule_pending_plugin_prompt(self) -> None:
        """初始化完成后，若有待安装的 .cwplugin 文件则弹出安装确认。"""
        if not self._pending_plugin:
            return
        QTimer.singleShot(600, self._show_pending_plugin_prompt)

    def _show_pending_plugin_prompt(self) -> None:
        """打开设置窗口并弹出插件安装确认对话框。"""
        from src.core.plugin.install_dialog import show_plugin_install_dialog

        # 先打开设置窗口（用户视角：跳转到设置）
        self.window_manager.open_settings()
        # 短暂延迟确保设置窗口已显示，再弹出安装确认
        QTimer.singleShot(300, lambda: show_plugin_install_dialog(self._pending_plugin, self))
