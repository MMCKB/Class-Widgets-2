...
class SettingsPlazaCentral(AppCentral):
    ...
    def __init__(self, pending_plugin: str | None = None) -> None:
        super().__init__(pending_plugin=pending_plugin)
...
def main(mode: Literal["settings", "plaza", "both"] = "both", pending_plugin: str | None = None) -> int:
    ...
    central = SettingsPlazaCentral(pending_plugin=pending_plugin)
    ...
    # 若有待安装的 .cwplugin 文件，弹出安装确认
    if pending_plugin:
        QTimer.singleShot(500, lambda: _show_pending_plugin_dialog(pending_plugin))
...
def _show_pending_plugin_dialog(archive_path: str) -> None:
    """弹出插件安装确认对话框。"""
    from src.core.plugin.install_dialog import show_plugin_install_dialog

    show_plugin_install_dialog(archive_path)
